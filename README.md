...
- [AviUtl ExEdit2 v2.1.10](https://spring-fragrance.mints.ne.jp/aviutl/)
...
> [!NOTE]
> 非連続なフレームが処理された場合は必要なフレームの最新キャッシュからデータを取得する．キャッシュがない場合はブラーはかからない．
...
> [!TIP]
> グループ制御の同一性は座標変換の階層で判定しているため，グループ制御の分割やレイヤージャンプがある場合でも補間される．

> [!WARNING]
> ブラー対象オブジェクトとそれに影響を与えるグループ制御は同一の時間制御(オブジェクト)の範囲内にあること．
