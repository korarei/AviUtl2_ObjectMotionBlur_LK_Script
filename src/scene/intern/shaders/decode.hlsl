...
Texture2D<int2> flow_maps[2] : register(t0);
Texture2D<uint> cost_maps[2] : register(t2);
...
inline float3 LoadFlow(uint index, int2 loc) {
    const float2 flow = flow_maps[index].Load(int3(loc, 0));
    const float cost = cost_maps[index].Load(int3(loc, 0));
    return float3(flow * rcp(32.0), cost * rcp(255.0));
}

float3 SampleFlow(uint index, float2 pos, uint2 size) {
    const SampleCoords coords = GetSampleCoords(pos, size);

    const float3 f00 = LoadFlow(index, coords.position[0][0]);
    const float3 f10 = LoadFlow(index, coords.position[1][0]);
    const float3 f01 = LoadFlow(index, coords.position[0][1]);
    const float3 f11 = LoadFlow(index, coords.position[1][1]);
...
float4 main(float4 pos : SV_Position) : SV_Target {
    uint2 size;
    flow_maps[0].GetDimensions(size.x, size.y);

    pos.xy -= 0.5;
    const float3 fwd = SampleFlow(0, pos.xy, size);
    const float2 dst = pos.xy + fwd.xy;
    const float3 bwd = SampleFlow(1, dst, size);
    ...
    return float4(fwd.xy * scale, lerp(confidence.x, sqrt(confidence.x * confidence.y) * agreement, mask), consistency);
}
